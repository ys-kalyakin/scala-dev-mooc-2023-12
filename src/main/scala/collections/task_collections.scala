...
object task_collections {
    ...
    def capitalizeIgnoringASCII(text: List[String]): List[String] = {
        text.zipWithIndex.map({
            case (e, idx) if idx > 0 => if (isASCIIString(e)) e.toUpperCase else e.toLowerCase
            case (e, _) => e
        })
    }
    ...
    def numbersToNumericString(text: String): String = {
        ...
        text.map(c => if (charMap.contains(c)) charMap(c) else c)
          .foldLeft(new StringBuilder())((l, r) => l.append(r))
          .toString()
    ...
    def intersectionAuto(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = {
        Set.from(dealerOne).concat(dealerTwo)
    }
    ...
    def filterAllLeftDealerAutoWithoutRight(dealerOne: Iterable[Auto], dealerTwo: Iterable[Auto]): Iterable[Auto] = {
        val dealerTwoSet = Set.from(dealerTwo)
        Set.from(dealerOne).diff(dealerTwoSet)
    }
}